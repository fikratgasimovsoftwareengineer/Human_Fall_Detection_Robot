...
(facenetLogger gLogger, DataType dtype, const string uffFile, const string engineFile, int batchSize, bool serializeEngine,
...
std::map<std::string, int> FaceNetClassifier::featureMatching(cv::Mat &image) {

    //AP
    //declare and initialize the output map
    std::map<std::string, int> personReco;
    for (int j = 0; j < m_knownFaces.size(); j++) {
      personReco.insert(pair<std::string, int>(m_knownFaces[j].className.c_str(), 0));
    }
    //AP: end
    ...
    for(int i = 0; i < (m_embeddings.size()/128); i++) {
        ...
        //cv::rectangle(image, cv::Point(m_croppedFaces[i].y1, m_croppedFaces[i].x1), cv::Point(m_croppedFaces[i].y2, m_croppedFaces[i].x2), cv::Scalar(0,0,255), 2,8,0);
	    ...
	    //AP
            std::cout << m_knownFaces[winner].className.c_str() << std::endl;

	    std::map<std::string, int>::iterator it = personReco.find(m_knownFaces[winner].className.c_str());
	    if (it != personReco.end())
	        it->second = 1;
            //AP: end
        }
        else if (minDistance > m_knownPersonThresh || winner == -1){
            cv::putText(image, "Unknown", cv::Point(m_croppedFaces[i].y1+2, m_croppedFaces[i].x2-3),
                    cv::FONT_HERSHEY_DUPLEX, 0.1 + 2*fontScaler ,  cv::Scalar(255,0,0), 1);
            cv::rectangle(image, cv::Point(m_croppedFaces[i].y1, m_croppedFaces[i].x1), cv::Point(m_croppedFaces[i].y2, m_croppedFaces[i].x2), 
